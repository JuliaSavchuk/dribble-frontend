...
export const useProfile = () => {
  ...
  return useQuery({
    ...
    staleTime: 1000 * 60 * 5,
    // Захист від виклику з компонентів, що рендеряться і для
    // неавторизованих сторінок (наприклад, Navbar) — без цього був би
    // зайвий 401-запит на /auth/profile/ для гостей.
    enabled: !!user,
    // КРИТИЧНО: initialData (застарілий user з localStorage) інакше
    // вважається "свіжим" і staleTime блокує реальний запит до 5 хв —
    // тобто прострочений підписаний URL аватара з R2 показувався б
    // без оновлення навіть після перезавантаження сторінки. З
    // refetchOnMount: 'always' сторінка одразу рендериться зі старими
    // даними (без миготіння), але тут-таки у фоні йде свіжий запит,
    // який виправляє URL протягом секунди.
